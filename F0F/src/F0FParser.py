...
class LL1_Parser(F0FParser):
    def __init__(self, tokens: list, grammar:Grammar):
    ...
    def begin(self):
        ...
        cursor = 0
        self.left_parse = []
        # self.lookahead = self.lexer_tokens[self.cursor].token_type
        ok, self.left_parse,_ = self.branching_parser(stack,cursor,self.left_parse)
    ...
    def error(self,token, msg,cursor):
        ...
        self.had_error = True
        self.parser_errors.append(ParsingError(token,msg))
        # return self.synchronize(cursor)
...
# source = open('F0F/src/1st_test.txt','r')
# code = source.read()
# source.close()
